...
class HCF_NN:
    def __init__(self, t, z, theta, psi, WRC_weights, WRC_biases, layers_K, layers_psi, lb, ub):
        ...
        self.loss_phys = tf.reduce_mean(tf.square(self.f_pred2))                     # PDE
    ...
    def net_K(self, X, weights, biases):
        ...
        W = weights[-1]
        b = biases[-1]
        K = 0.26 * tf.sigmoid(tf.add(tf.matmul(H, W), b))  # range (0, 2)
        return K
...
def main_loop(hydrus, noise, num_layers_psi, num_neurons_psi, num_layers_theta, num_neurons_theta, num_layers_K, num_neurons_K, number_random, lb, ub, sensorN):
    ...
    # --- 3. Setup Folders ---
    folder_path = f"./Interval40/Interval{lb}-{ub}-{sensorN}"
...
# ======================
# Data
intervals = [(10 + 10 * i, 50 + 10 * i) for i in range(5)]


counts_dict = {
    (10 + 10 * i, 50 + 10 * i): [5, 7, 9, 11, 13, 15, 17, 19]
    for i in range(7)
}
